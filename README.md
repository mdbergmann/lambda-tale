...
- `floor.iff` / `ceiling.iff` and `ground.iff` / `sky.iff` — the
  Bard's Tale split backdrop: one image fills the view above the
  ...
  **Put the ceiling on pen 5 and the floor on pen 6**, whichever pair
  ...
  `worlds/closure/make-dungeon-packs.lisp` (three dungeon packs, one
  painted stone each, the same flagged floor in all of them) is the
  worked example that keeps the contract.
...
(cookie-cut, RTG-safe); the backdrops are opaque, so pen 0 there is
plain black.

The Closure game ships worked examples, all declared map-relative and
all generated: [`worlds/closure/gfx/`](../closure-tale/worlds/closure/gfx)
— the town's painted houses under a day-banded sky, from its own
`make-pack.lisp` — and three dungeon packs,
[`gfx-rubble/`](../closure-tale/worlds/closure/gfx-rubble),
[`gfx-ashlar/`](../closure-tale/worlds/closure/gfx-ashlar) and
[`gfx-rock/`](../closure-tale/worlds/closure/gfx-rock), one painted
stone each over the same flagged floor and ceiling, from one
`make-dungeon-packs.lisp`; each dungeon zone names the stone it wears.
...
together — city gates, stairs and portals are all map data (a stair
that asks before it is taken wraps its travel in the `ask` op, see
"Asking first" below).  A world
...
`event`, `ask` — is documented in `src/specials.lisp`.
...
across, and a descent is a decision.  `(ask TEXT... OP...)` is the
pause — Bard's Tale's *"Stairs down.  Take them?"* — as map data: the
leading strings are the question (free prose; the page wraps it), and
the ops after them run only on a yes:
...
The question is a box both front-ends draw over the play page, the
quit confirmation's shape (`question-lines` / `question-act`): `Y`
takes the offer, `N` or `Esc` declines and the party simply stands on
the cell, `Q` still asks to quit.  It eats every other key until it is
answered, pauses the Amiga's idle clock, and a step that ends on a
question draws no wandering roll — the party has stopped to decide.
Ops after the `ask` on the same cell run at once; a `once` belongs
inside the `ask`, not around it, or a no would spend it.
